...
class Timezz {
    constructor(element, userSettings) {
        ...
        this.HTMLParts = partNames.reduce((acc, part) => ({
            ...acc,
            [part]: null,
        }), {});
        this.timeout = null;
        this.checkFields = (settings) => {
            const warn = (field, types) => {
                if (settings[field] !== undefined && types.length) {
                    // eslint-disable-next-line no-console
                    console.warn(`${TIMEZZ}:`, `Parameter '${field}' should be ${types.length > 1 ? 'one of the types' : 'the type'}: ${types.join(', ')}.`, this.element);
                }
            };
...
exports.Timezz = Timezz;
const timezz = (element, settings) => {
